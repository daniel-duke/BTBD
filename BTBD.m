...
%%% set output
outFold = "/Users/dduke/Files/triarm/monomer/lammps/";
nsim = 1;
...
function write_input(inputFile,reactFold,p,nABADtype,ls,rs,pots,apots,dpots)
    ...
    if p.nstep > 0
        ...
        if ~isempty(rs)
            fprintf(f,strcat(...
            "compute         compBR1 sticky bond/local dist engpot\n",...
            "compute         compBR2 sticky property/local btype batom1 batom2\n",...
            "dump            dumpBR sticky local ", num2str(p.dump_every), " dump_bondReact.dat index c_compBR1[1] c_compBR1[2] c_compBR2[1] c_compBR2[2] c_compBR2[3]\n"));
        end
        if nABADtype(3) > 0
            fprintf(f,strcat(...
